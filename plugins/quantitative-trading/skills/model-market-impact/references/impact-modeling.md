...
Use only values available when the estimate would have been produced:
...
Avoid including realized volume, realized volatility, or final schedule
statistics in a pre-trade model unless they are explicitly lagged or forecast.
...
For black-box response mapping, build a bounded synthetic grid, vary one input
at a time, repeat requests to measure noise, and fit a surrogate only within the
